...
const ChartHistory = () => {
  ...
  // State to control modal and chart viewing
  ...
  useEffect(() => {
  fetch("http://localhost:3000/history/uploads")
    .then(res => {
...
const handleDelete = async (id) => {
  ...
  try {
    const res = await fetch(`http://localhost:3000/api/uploads/${id}`, {
      method: "DELETE",
    });
  ...
  // Generate example chart data based on file info and selected chart type
  const generateChartData = (file, chartTypeKey) => {
    // Just a dummy example using file name length for data points
  ...
  const handleViewClick = (file) => {
    ...
    setSelectedChartType(null); // Reset chart type selection
    setShowModal(true);
  };
  ...
  return (
    ...
    <div className="md:w-2xl md:mx-20 lg:w-6xl lg:mx-70 mt-12 p-6 bg-gray-100 rounded-2xl">
      ...
      {loading ? (
        <div className="text-gray-600 py-16 text-center">Loading history...</div>
      ) : err ? (
        <div className="bg-red-100 border border-red-400 text-red-700 p-4 rounded">{err}</div>
      ) : history.length === 0 ? (
        ...
        >
          ...
          >
            ...
            {!selectedChartType ? (
              ...
              <>
                <h3 className="text-xl font-semibold mb-4 text-center">
                  {chartTypes.find((c) => c.key === selectedChartType)?.label} for "{selectedFile?.originalname}"
                </h3>
                <div className="h-72">
                  <SelectedChartComponent
                    ref={chartRef}
